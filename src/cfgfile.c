...
static TCHAR *cfgfile_unescape_sep (const TCHAR *s, const TCHAR **endpos, TCHAR separator)
{
	bool quoted = false;
...
{
	return cfgfile_unescape_sep (s, endpos, 0);
}
...
static TCHAR *getnextentry (const TCHAR **valuep, const TCHAR separator)
{
	...
	if (value[0] == '\"') {
		...
		*valuep = value;
		...
		s = cfgfile_unescape_sep (value, valuep, separator);
	}
	return s;
}
...
static int cfgfile_parse_hardware (struct uae_prefs *p, const TCHAR *option, TCHAR *value)
{
	...
	if (cfgfile_path_mp (option, value, _T("kickstart_rom_file"), p->romfile, sizeof p->romfile / sizeof (TCHAR), &p->path_rom)
		...
		|| cfgfile_path_mp (option, value, _T("a2091_2_rom_file"), p->a2091romfile2, sizeof p->a2091romfile2 / sizeof (TCHAR), &p->path_rom)
		|| cfgfile_path_mp (option, value, _T("a4091_rom_file"), p->a4091romfile, sizeof p->a4091romfile / sizeof (TCHAR), &p->path_rom)
		|| cfgfile_path_mp (option, value, _T("a4091_2_rom_file"), p->a4091romfile2, sizeof p->a4091romfile2 / sizeof (TCHAR), &p->path_rom)
