...
int nr_directory_units (struct uae_prefs *p)
{
	...
	if (p) {
		...
		for (i = 0; i < MAX_FILESYSTEM_UNITS; i++) {
			if (mountinfo.ui[i].open && mountinfo.ui[i].controller_type == HD_CONTROLLER_TYPE_UAE)
				cnt++;
		}
	}
	return cnt;
...
#ifdef SCSI_TAPE
	for (nr = 0; nr < currprefs.mountitems; nr++) {
		struct uaedev_config_data *uci = &currprefs.mountconfig[nr].ci;
		int type = uci->controller_type;
		int unit = uci->controller_unit;
		bool added = false;
		if (type == HD_CONTROLLER_TYPE_UAE) {
			continue;+		} else if (type >= HD_CONTROLLER_TYPE_IDE_FIRST && type <= HD_CONTROLLER_TYPE_IDE_LAST) {
